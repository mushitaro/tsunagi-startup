...
このリポジトリは `startup.tsunagi.app` を入り口とする tsunagi の統合サイト（Catalog / Content / Consulting）です。
AIエージェントがコンテンツ作成・プレビュー・修正・デプロイを行うことを前提に構成しています。

## このサイトの構成

- **Catalog**（`/catalog`）: アプリ・開発中リポジトリのポートフォリオ
- **Content**（`/content`）: 記事。`access: paid` で Stripe による記事単位課金
- **Consulting**（`/consulting`）: アプリのデザイン・開発・運用受託。問い合わせは Discord

日本語が既定。英語版は各 URL の `/en/` 配下（例: `/en/catalog`）。
...
2. ファイル名 `<slug>` がそのまま URL（`/catalog/<slug>`）になる。
